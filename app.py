...
def draw_lives(surf, lives):
    surf.blit(pygame.font.SysFont("dejavusansmono", 28).render("❤ "*lives, True, (255,0,0)),
              (10, 10))
...
def draw_start_screen(surf, demo_pipes, demo_land, bg_color):
    ...
    font_big = pygame.font.SysFont("ubuntumono", 48, bold=True) or pygame.font.SysFont(None, 48)
...
def draw_zebra_active(surf, ms_left):
    font = pygame.font.SysFont("ubuntumono", 32) or pygame.font.SysFont(None, 32)
    txt = font.render(f"ZEBRA SPEED! {max(0, ms_left//1000)}s", True, (0,0,0))
